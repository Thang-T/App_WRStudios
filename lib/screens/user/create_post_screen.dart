...
class _CreatePostScreenState extends State<CreatePostScreen> {
  ...
  Future<void> _pickImages() async {
    // 1. Kiểm tra Web trước để tránh lỗi dart:io Platform
    if (kIsWeb) {
      try {
        final images = await ImagePickerService.pickMultipleImages();
        if (images.isNotEmpty) {
          _handlePickedImages(images);
        }
      } catch (e) {
        _showError(AppLocalizations.of(context)!.photoPickError(e.toString()));
      }
      return;
    }

    // 2. Xử lý iOS (Simulator/Physical) dùng CustomImagePicker
    if (Platform.isIOS) {
      ...
      if (files != null && files.isNotEmpty) {
        _handlePickedImages(files.map((f) => XFile(f.path)).toList());
        // AI Analyze
  ...
  void _handlePickedImages(List<XFile> images) {
    if (!mounted) return;
    
    // Manual limit check
    if (_selectedImages.length + images.length > 5) {
      _showError(AppLocalizations.of(context)!.maxImagesLimit(5));
      return;
    }
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                if (index < _existingImages.length) {
                  ...
                  final xFile = _selectedImages[newIndex];
